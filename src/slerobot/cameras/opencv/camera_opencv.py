...
from slerobot.utils.decorators import check_if_already_connected, check_if_not_connected
from slerobot.utils.errors import DeviceNotConnectedError
...
class OpenCVCamera(Camera):
    ...
    def _validate_fps(self) -> None:
        ...
        # Some cameras/backends expose only fixed FPS modes and silently fall back
        # to the closest supported value (common on macOS/AVFoundation). In that case
        # keep running and adopt the real FPS instead of failing hard.
        if not math.isclose(requested_fps, actual_fps, rel_tol=1e-3):
            logger.warning(
                "%s requested fps=%s but camera reports actual_fps=%s; using actual FPS.",
                self,
                requested_fps,
                actual_fps,
            )
    ...
    def _validate_width_and_height(self) -> None:
        ...
        if not width_success or self.capture_width != actual_width:
            logger.warning(
                "%s requested capture_width=%s but camera reports actual_width=%s; using actual resolution.",
                self,
                self.capture_width,
                actual_width,
            )
        ...
        if not height_success or self.capture_height != actual_height:
            logger.warning(
                "%s requested capture_height=%s but camera reports actual_height=%s; using actual resolution.",
                self,
                self.capture_height,
                actual_height,
            )
