...
from slerobot.utils.constants import HF_LEROBOT_HOME
from slerobot.datasets.compute_stats import compute_episode_stats,aggregate_stats
from slerobot.datasets.image_writer import write_image, AsyncImageWriter
from slerobot.datasets.utils import (
...
from slerobot.datasets.video_utils import (
...
class sLerobotDataset(torch.utils.data.Dataset):
    def __init__(
    self,
    repo_id:str,
    ...
    encoder_threads: int | None = None,
        ...
        """  To instantiatise this class:
        1) dataset exists:
            -on local, load with 'root'
            -on hub, load with 'repo_id'
        2) dataset doesn't exists
            - create sLeRobotDataset with 'create' classmethod

        What recorded:
        1) metadata:
            - info, keys/shapes/fps
            - stats, dataset statistics for normalization
            - tasks, prompt
        2) hf_dataset:
            - read any values from parquet files
        3) videos:
            - where frames are loaded to be synchronous with data from parquet
    ...
    #================================= INITIALIZATION  =================================
        # Load metadata
        self.meta = sLerobotDatasetMetadata(
    ...
    def create(
        ...
        # Create a sLeRobot Dataset from scratch in order to record data.
        vcodec = resolve_vcodec(vcodec)
        obj = cls.__new__(cls)
        obj.meta = sLerobotDatasetMetadata.create(
            repo_id=repo_id,
            fps=fps,
...
class sLerobotDatasetMetadata:
    ...
    # 2. Getters for metadata attributes
    @property
    ...
    # 3. Getters for metadata statistics
    @property
