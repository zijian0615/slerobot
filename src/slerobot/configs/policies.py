...
from slerobot.utils.constants import ACTION, OBS_STATE
from slerobot.utils.hub import HubMixin
from slerobot.utils.utils import auto_select_torch_device, is_amp_available, is_torch_device_available
...
class PreTrainedConfig(draccus.ChoiceRegistry, HubMixin, abc.ABC):  # type: ignore[misc,name-defined] #TODO: draccus issue
    ...
    def from_pretrained(
        ...
        **policy_kwargs: Any,
        ...
        # 支持本地文件和目录
        if path_obj.is_file():
            config_file = str(path_obj)
        elif path_obj.is_dir():
            if CONFIG_NAME in os.listdir(model_id):
                config_file = os.path.join(model_id, CONFIG_NAME)
            else:
                logger.error(f"{CONFIG_NAME} not found in {path_obj.resolve()}")
        else:
            try:
